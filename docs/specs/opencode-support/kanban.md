...
- **Type:** feature
- **Status:** done (done ahead of T-109 — see note)
...
- 2026-07-16: T-110 done (ahead of T-109 — docs describe only shipped behavior). Both `README.md` and `README.zh-CN.md` updated for multi-backend: intro reframed to "Claude Code + OpenCode"; a prominent **zero-residue** callout near the top; Features gained a Multi-Backend bullet + per-backend avatar shape (circle vs rounded-square); the .dmg prerequisite now lists Claude Code and OpenCode as independently-optional (at least one required), with the verified OpenCode install command (`curl -fsSL https://opencode.ai/install | bash`, confirmed against opencode.ai/docs); Usage Guide covers the backend selector + missing-CLI warning + same-cwd-both-backends in "Creating an instance", the layout diagram/labels are backend-neutral, the Quick Actions table notes Show Cost disabled on opencode and the per-backend Resume command, and a new **View** section documents the Markdown viewer (also fills a prior doc gap). Notification/How-It-Works reworded to "completion detected per backend (JSONL vs sqlite), read-only". Also corrected two stale facts: removed the deleted `session-watcher.ts` from the structure tree and added `backends/`. Cross-links verified both directions. **Deliberately does NOT mention wait-for-input notifications** (T-107 not done) — only completion notifications are described, which both backends have. type/lint not applicable (docs only); no code touched. Remaining: T-107 (opencode wait-for-input, paused pending a live signal probe) and T-109 (manual E2E QA, needs a running app).
- 2026-07-16: T-108 done. Quick Actions is now backend-aware. **Resume Elsewhere** no longer hardcodes `claude --resume`: a new `get-resume-command` IPC looks up the instance, and `getBackend(instance.backend).buildResumeCommand(sessionId)` builds the string in main (claude → `claude --resume <id>`, opencode → `opencode --session <id>`) — the renderer stays backend-agnostic and just copies it. **Show Cost** is disabled on opencode with tooltip "OpenCode does not have an inline cost command" (`disabled={!isRunning || isOpencode}`); this one backend conditional in the renderer is sanctioned by the task since the behavior is fundamentally different (disabled vs slash). Clear/Compact/Go-to-Code-Base unchanged. Resume stays disabled until sessionId is known (unchanged). Added `getResumeCommand` to preload + `ElectronAPI` type. type/main-build/lint/renderer-build green; `buildResumeCommand` output runtime-verified for both backends via the compiled registry (`claude --resume abc-123`, `opencode --session xyz-789`). GUI click/copy needs eyeballing in a running app. T-107 remains the last M1-feature gap; T-109 now unblocked once T-107 lands.
